...
import { appConfig, ISOFFLINE, OFFLINE_LOGIN_DATA } from '@dd/common';
import {} from 'jasmine'
...
describe('ConfigurationInitializer::', () => {
  ...
  it('getFilePath should retun correct path for login.json', () => {
    const filepath = initializer.getFilePath(LOGIN_JSON)
  ...
  it('onResolved  should insert correct value in app-config for new files', () => {
    ...
    initializer.onResolved('test.json', {"name": "Arpit"})
  ...
  it('onResolved  should insert correct value in offline-data for login', () => {
    ...
    initializer.onResolved(LOGIN_JSON, {"name": "Arpit"})
  ...
  it('onResolved  should insert correct value in app-config for config.json', () => {
    ...
    initializer.onResolved('config.json', {"appName": "Arpit"})
  ...
  it('run', async(() => {
    ...
    files_count = 1;
    if(ISOFFLINE) {
      const loginpath = initializer.getFilePath(LOGIN_JSON);
