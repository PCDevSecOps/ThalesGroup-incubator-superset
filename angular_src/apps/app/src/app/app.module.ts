...
import {
  AuthInterceptor,
  AuthTokenService
  } from '@guavus/ui-kit-auth-interceptor';
import {
  ABOUT_APP,
  ComponentsModule
  } from '@guavus/ui-kit-components';
import {
  ErrorHandler,
  ErrorInterceptor,
  ErrorMessages,
...
import { MatAlertComponent } from '@guavus/ui-kit-components';
...
@NgModule({
  imports: [
    ...
    RouterModule.forRoot(applicationRoutes, { enableTracing: TRACE_ROUTES, initialNavigation: 'disabled' }),
...
export function errorHandlerFactory(store: Store<RootState>, messages: ErrorMessages, dialog: MatDialog) {
  ...
  errorHandler.setErrorPopup(MatAlertComponent);
  return errorHandler;
}
